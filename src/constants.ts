...
export namespace ToolTips {
  ...
  // Controls
  export const MOVE =
    t(`Use these manual control buttons to move FarmBot in realtime. Press the
  ...
  export const WEBCAM_SAVE =
    t(`Press the edit button to update and save your webcam URL.`);

  export const PERIPHERALS =
    t(`Use these toggle switches to control FarmBot's peripherals in realtime.
  ...
  export const OS_SETTINGS =
    t(`View and change device settings.`);

  export const HW_SETTINGS =
    t(`Change settings of your FarmBot hardware with the fields below. Caution:
  ...
  export const HOMING =
    t(`(Alpha) If encoders or end-stops are enabled, home axis (find zero).`);

  export const CALIBRATION =
    t(`(Alpha) If encoders or end-stops are enabled, home axis and determine
    maximum.`);
  ...
  export const FIND_HOME_ON_BOOT =
    t(`If encoders or end-stops are enabled, find the home position when the
    device powers on.`);

  export const STOP_AT_HOME =
    t(`Stop at the home location of the axis.`);

  export const STOP_AT_MAX =
    t(`Don't allow movement past the maximum value provided in AXIS LENGTH.`);

  export const NEGATIVE_COORDINATES_ONLY =
    t(`Restrict travel to negative coordinate locations. Overridden by disabling
    STOP AT HOME.`);

  export const LENGTH =
    t(`Set the length of each axis to provide software limits. Used only if
    STOP AT MAX is enabled.`);

  export const TIMEOUT_AFTER =
    t(`Amount of time to wait for a command to execute before stopping.`);

  // Hardware Settings: Motors
  export const MAX_MOVEMENT_RETRIES =
    t(`Number of times to retry a movement before stopping.`);

  export const MAX_SPEED =
    t(`Maximum travel speed after acceleration in motor steps per second.`);

  export const MIN_SPEED =
    t(`Minimum movement speed. Also used for homing, calibration, and movements
    across home.`);

  export const ACCELERATE_FOR =
    t(`Number of steps used for acceleration and deceleration.`);

  export const STEPS_PER_MM =
    t(`The number of motor steps required to move the axis one millimeter.`);

  export const ALWAYS_POWER_MOTORS =
    t(`Keep power applied to motors. Prevents slipping from gravity in certain
    situations.`);
  ...
  export const ENABLE_X2_MOTOR =
    t(`Enable use of a second x-axis motor. Connects to E0 on RAMPS.`);

  // Hardware Settings: Encoders and Endstops
  export const ENABLE_ENCODERS =
    t(`(Alpha) Enable use of rotary encoders during calibration and homing.`);

  export const ENCODER_POSITIONING =
    t(`[EXPERIMENTAL] Use encoders for positioning.`);

  export const INVERT_ENCODERS =
    t(`(Alpha) Reverse the direction of encoder position reading.`);

  export const MAX_MISSED_STEPS =
    t(`(Alpha) Number of steps missed (determined by encoder) before motor is
    considered to have stalled.`);

  export const ENCODER_MISSED_STEP_DECAY =
    t(`(Alpha) Reduction to missed step total for every good step.`);

  export const ENCODER_SCALING =
    t(`(Alpha) encoder scaling factor = 100 * (motor resolution * microsteps) /
    (encoder resolution)`);

  export const ENABLE_ENDSTOPS =
    t(`Enable use of electronic end-stops during calibration and homing.`);
  ...
  export const FARMWARE =
    t(`Manage Farmware (plugins).`);

  export const PHOTOS =
    t(`Take and view photos with your FarmBot's camera.`);

  export const WEED_DETECTOR =
    t(`Detect weeds using FarmBot's camera and display them on the Farm Designer
    map.`);

  export const CAMERA_CALIBRATION =
    t(`Calibrate FarmBot's camera for use in the weed detection software.`);

  // Sequences
  export const SEQUENCE_COMMANDS =
    t(`These are the most basic commands FarmBot can execute. Drag and drop them
    to create sequences for watering, planting seeds, measuring soil properties,
    and more.`);

  export const SEQUENCE_EDITOR =
    t(`Drag and drop commands here to create sequences for watering, planting
  ...
  export const SEQUENCE_LIST =
    t(`Here is the list of all of your sequences. Click one to edit.`);

  export const MOVE_ABSOLUTE =
    t(`The Move Absolute step instructs FarmBot to move to the specified
  ...
  export const MOVE_RELATIVE =
    t(`The Move Relative step instructs FarmBot to move the specified distance
  ...
  export const WRITE_PIN =
    t(`The Write Pin step instructs FarmBot to set the specified pin on the
  ...
  export const READ_PIN =
    t(`The Read Pin step instructs FarmBot to read the current value of the
    specified pin. A Pin Mode of 0 is for digital (on/off), while a Pin Mode
    of 1 is for analog (0-1023 for 0-5V).`);

  export const WAIT =
    t(`The Wait step instructs FarmBot to wait for the specified amount of time.
    Use it in combination with the Pin Write step to water for a length of
    time.`);

  export const SEND_MESSAGE =
    t(`The Send Message step instructs FarmBot to send a custom message to the
    logs (and toast message and/or email, if selected). This can help you with
    debugging your sequences.`);

  export const FIND_HOME =
    t(`The Find Home step instructs the device to perform a homing command to
    find and set zero for the chosen axis or axes.`);

  export const IF =
    t(`Execute a sequence if a condition is satisfied. If the condition is not
    satisfied, chose to do nothing or execute a different sequence.`);

  export const EXECUTE_SCRIPT =
    t(`The Run Farmware step runs a Farmware package. The weed detection script
    is the only script supported at the moment, but user definable script
    support is coming soon!`);

  export const TAKE_PHOTO =
    t(`Snaps a photo using the device camera. Select the camera type on the
    Device page.`);

  // Regimens
  export const BULK_SCHEDULER =
    t(`Add sequences to your regimen by selecting a sequence from the drop down,
  ...
  export const REGIMEN_EDITOR =
    t(`Regimens allow FarmBot to take care of a plant throughout its entire
    life. A regimen consists of many sequences that are scheduled to run based
    on the age of the plant. Regimens are applied to plants from the farm
    designer (coming soon) and can be re-used on many plants growing at the same
    or different times. Multiple regimens can be applied to any one plant.`);

  export const REGIMEN_LIST =
    t(`This is a list of all of your regimens. Click one to begin editing it.`);

  // Tools
  export const TOOL_LIST =
    t(`This is a list of all your FarmBot Tools. Click the Edit button to add,
    edit, or delete tools.`);

  export const TOOLBAY_LIST =
    t(`Toolbays are where you store your FarmBot Tools. Each Toolbay has Slots
...
export namespace Content {
  ...
  export const ACCOUNT_DELETE_WARNING =
    t(`WARNING! Deleting your account will permanently delete all of your
  ...
  export const FACTORY_RESET_WARNING =
    t(`Factory resetting your FarmBot will destroy all data on the device,
